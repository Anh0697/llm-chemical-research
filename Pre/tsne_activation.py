...
# Paths
ACTIVATIONS_DIR = '/mnt/d/Anh Nguyen/llm-chemical-research/activation_datasets_3/meta-llama-Meta-Llama-3.1-70B/atomic number/'
...
def get_layer_files(directory):
    ...
    """
    files = []
    pattern = re.compile(r'atomic number\.last\.\d+_templates\.layer_(\d+)\.pt')
...
def main():
    ...
    if SELECTED_LAYERS is not None:
        layer_files = [f for f in layer_files if int(re.search(r'atomic number\.last\.\d+_templates\.layer_(\d+)\.pt', f).group(1)) in SELECTED_LAYERS]
        if not layer_files:
    ...
    for idx, filename in enumerate(layer_files):
        ...
        match = re.search(r'atomic number\.last\.\d+_templates\.layer_(\d+)\.pt', filename)
        if not match:
